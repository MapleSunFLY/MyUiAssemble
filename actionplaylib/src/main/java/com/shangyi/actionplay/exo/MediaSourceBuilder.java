...
public final class MediaSourceBuilder {
     ...
     * @param item     视频
     ***/
    <T extends VideoResModel> void setMediaSourceItem(@NonNull Context context, @NonNull T item) {
        this.context = context;
        mainHandler = new Handler();
        initData(item);
    }
     ...
     * @param uris     视频的地址列表
     ***/
    <T extends VideoResModel> void setMediaSourceUri(@NonNull Context context, @NonNull List<T> uris) {
     ...
     * @param uris     视频的地址列表
     ***/
    <T extends VideoResModel> void setMediaSourceItem(@NonNull Context context, @NonNull List<T> uris) {
        MediaSource[] firstSources = new MediaSource[uris.size()];
        this.context = context;
        mainHandler = new Handler();
        int i = 0;
    ...
    private void initDataConcatenatingMediaSource(int streamType, Uri firstVideoUri, Uri secondVideoUri) {
        ...
        switch (streamType) {
            case C.TYPE_SS:
                MediaSource secondSource = new SsMediaSource(secondVideoUri,
     ...
     * @param item        视频对象
     ***/
    private MediaSource initData(VideoResModel item) {
        if (item.getVideoUri() == null) {
            throw new NullPointerException("NullPointerException VideoResModel");
        }
        Uri uri = Uri.parse(item.getVideoUri());
        ...
        switch (streamType) {
            ...
            case C.TYPE_DASH:
                ...
                break;
            case C.TYPE_HLS:
                mediaSource = new HlsMediaSource.Factory(new DefaultHlsDataSourceFactory(getDataSource())).createMediaSource(uri);
                break;
            case C.TYPE_OTHER:
                mediaSource = new ExtractorMediaSource.Factory(getDataSource()).createMediaSource(uri);
                break;
            default:
                throw new IllegalStateException("Unsupported type: " + streamType);
        }
