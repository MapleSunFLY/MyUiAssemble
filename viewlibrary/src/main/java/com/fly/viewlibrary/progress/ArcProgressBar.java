...
public class ArcProgressBar extends View {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        finishedStrokeColor = typedArray.getColor(R.styleable.ArcProgressBar_arcFinishedColor, DEFAULT_FINISHED_COLOR);
        unfinishedStrokeColor = typedArray.getColor(R.styleable.ArcProgressBar_arcUnfinishedColor, DEFAULT_UNFINISHED_COLOR);
        max = typedArray.getInteger(R.styleable.ArcProgressBar_arcProgressMax, 100);
        progress = typedArray.getInteger(R.styleable.ArcProgressBar_arcProgress, 0);
        arcFullDegree = typedArray.getInteger(R.styleable.ArcProgressBar_arcFullDegree, ARC_FULL_DEGREE);
        STROKE_WIDTH = typedArray.getInteger(R.styleable.ArcProgressBar_arcWidth, 0);
        animaTime = typedArray.getInteger(R.styleable.ArcProgressBar_animaTime, 500);
        draggingEnabled = typedArray.getBoolean(R.styleable.ArcProgressBar_draggingEnabled, false);
        describeText = typedArray.getString(R.styleable.ArcProgressBar_describeText);
        textColor = typedArray.getColor(R.styleable.ArcProgressBar_describeTextColor, DEFAULT_FINISHED_COLOR);
        typedArray.recycle();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (progress > 0) canvas.drawArc(circleRectF, startAngle, sweep1, false, progressPaint);
    ...
    private boolean checkOnArc(float currentX, float currentY) {
        ...
        return distance > circleRadius - STROKE_WIDTH * 5 //点击区域弧内五倍STROKE_WIDTH
                && distance < circleRadius + STROKE_WIDTH * 5//点击区域弧外五倍STROKE_WIDTH
                && (degree >= -8 &&//圆弧开始位置
    ...
    public void setProgressAnima(float progress) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                float oldProgress = ArcProgressBar.this.progress;
                for (float i = 1.0f; i <= 100f; i++) {
                    ArcProgressBar.this.progress = oldProgress + (thisProgress - oldProgress) * i / 100;
                    postInvalidate();
