...
public class StatisticsLineChartView extends View {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mLineColor = typedArray.getColor(R.styleable.StatisticsLineChart_statisticsLineColor, Color.parseColor("#39499C"));
        ...
        mMaxBackgroundColor = typedArray.getColor(R.styleable.StatisticsLineChart_statisticsMaxBackgroundColor, Color.parseColor("#39499C"));
        ...
        mLineHeight = typedArray.getDimension(R.styleable.StatisticsLineChart_statisticsLineHeight, ViewUtils.dip2px(getContext(), 1f));
        mTextColor = typedArray.getColor(R.styleable.StatisticsLineChart_statisticsLineTextColor, Color.WHITE);
        mTextSize = typedArray.getDimension(R.styleable.StatisticsLineChart_statisticsLineTextSize, ViewUtils.dip2px(getContext(), 7f));
        isShowMax = typedArray.getBoolean(R.styleable.StatisticsLineChart_statisticsShowMax, true);
        isShowMin = typedArray.getBoolean(R.styleable.StatisticsLineChart_statisticsShowMin,true );
        typedArray.recycle();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        boolean isMax = isShowMax;
        boolean isMin = isShowMin;
    ...
    private void drawBackground(Canvas canvas, float width, float height, float originX) {
        ...
        Path path = new Path();
        mDottedPaint.setPathEffect(new DashPathEffect(new float[]{ViewUtils.dip2px(getContext(), 4f), ViewUtils.dip2px(getContext(), 4f)}, 0));
        for (int i = 1; i <= 4; i++) {
            path.moveTo(originX, getPaddingTop() + height / 4 * i + ViewUtils.dip2px(getContext(), 5f) * (i-1));
            path.lineTo(originX + width, getPaddingTop() + height / 4 * i + ViewUtils.dip2px(getContext(), 5f) * (i-1));
            canvas.drawPath(path, mDottedPaint);
            if (i == 4) {
                mPaint.setColor(mSolidLineColor);
                mPaint.setStyle(Paint.Style.STROKE);
                canvas.drawLine(originX, getPaddingTop() + height / 4 * i + ViewUtils.dip2px(getContext(), 5f) * 3, originX + width, getPaddingTop() + height / 4 * i + + ViewUtils.dip2px(getContext(), 5f) * 3, mPaint);
